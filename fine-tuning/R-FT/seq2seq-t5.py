...
from transformers import AutoModelForCausalLM, AutoTokenizer, Trainer, TrainingArguments, DataCollatorForLanguageModeling, EarlyStoppingCallback, AutoModelForSeq2SeqLM
...
parser = argparse.ArgumentParser(description='Finetune Flan-t5')
...
parser.add_argument('--model-name', type=str, default='google/flan-t5-xl', help='model name')
parser.add_argument('--base-model-id', type=str, default='google/flan-t5-xl', help='base model id')
...
parser.add_argument('--cuda', type=str, default='2', help='cuda')
...
output_dir = f"/home/singh_shruti/temporal_ft/temporal_ft/models_t5/{base_model_id}-{ft_type}-finetuned-{datetime.now().strftime('%Y-%m-%d-%H-%M')}"
