...
for year_ in years_folder:
    ...
    os.system(f"CUDA_VISIBLE_DEVICES={args.cuda} python /path_to_lora_to_original.py/lora_to_original.py --model {args.hub_model_name} --adapter {checkpoint} --output /path_to_save_models_from_lora_to_original/{model_name}")

    # Run inference
    print(f"Running inference for {year}")
    os.system(f"CUDA_VISIBLE_DEVICES={args.cuda} python /path_to_code4.py/code4.py --model /path_to_save_models_from_lora_to_original/{model_name} --base-dir {args.base_dir} --out-dir {args.out_dir}/{model_name}/{year} --year {year} --hub-model-name {args.hub_model_name}")
